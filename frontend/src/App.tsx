...
function projectionLyricWeight(text: string) {
  return text
    .split('\n')
...
function splitProjectionLyrics(lyrics: string) {
  ...
  const useStanzas = stanzas.length > 1;
  const units = useStanzas ? stanzas : normalized.split('\n').map((part) => part.trim()).filter(Boolean);
  const joiner = useStanzas ? '\n\n' : '\n';
  const totalWeight = units.reduce((total, unit) => total + projectionLyricWeight(unit) + 1, 0);
  const columnCount = totalWeight > 68 ? 3 : totalWeight > 18 ? 2 : 1;
  ...
  for (const unit of units) {
    const weight = projectionLyricWeight(unit) + 1;
    if (columnIndex < columnCount - 1 && currentWeight > 0 && currentWeight + weight > targetWeight) {
      columnIndex += 1;
      currentWeight = 0;
    }
    columns[columnIndex].push(unit);
    currentWeight += weight;
  }

  return columns.map((column) => column.join(joiner)).filter(Boolean);
}
...
function PresentationsPage() {
  ...
  const [status, setStatus] = useState('');
  const [projectionZoom, setProjectionZoom] = useState(() => clampNumber(readLocal(PRESENTATION_ZOOM_KEY, 1.25), 0.85, 1.8));
      ...
      <section className="presentation-show">
        <header className="presentation-show-top">
          <div>
            ...
            <strong>Slide {selectedIndex + 1} of {songs.length}</strong>
          </div>
          <button className="ghost-action" onClick={() => setPresenting(false)}><X size={18} /> Exit</button>
        </header>
        <article className="presentation-slide">
          <p className="eyebrow">{slideSong.collectionName} / {songNumberLabel(slideSong)}</p>
          ...
          >
            {projectionLyricColumns.map((column, index) => (
              <pre key={`${slideSong.id}-column-${index}`}>{column}</pre>
            ))}
          </div>
        </article>
        <footer className="presentation-controls">
          <button className="ghost-action" disabled={selectedIndex === 0} onClick={() => setSelectedIndex(selectedIndex - 1)}>
          ...
          <button className="primary-action" disabled={selectedIndex >= songs.length - 1} onClick={() => setSelectedIndex(selectedIndex + 1)}>
